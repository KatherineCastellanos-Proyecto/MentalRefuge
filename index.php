...
if (isset($_POST['btn_ingresar'])) {
    ...
    $sql = "SELECT * FROM v3_ussa arios WHERE correo = '$correo' AND password = '$password'";
...
// LÓGICA DE RECUPERACIÓN CON TOKEN (6 DÍGITOS)
$fase_recuperar = 1; // 1: Pedir Correo, 2: Validar Código, 3: Nueva Clave

// Si el usuario da clic en "Olvidé mi clave"
if (isset($_GET['accion']) && $_GET['accion'] == 'recuperar') {
    $_SESSION['recuperando_fase'] = 1;
}

// PASO 1: El usuario ingresa su correo
if (isset($_POST['btn_enviar_codigo'])) {
    $correo_recuperar = mysqli_real_escape_string($conexion, $_POST['correo_recuperar']);
    
    // Verificar si el correo existe
    $sql_verificar = "SELECT * FROM v3_usuarios WHERE correo = '$correo_recuperar'";
    ...
    if (mysqli_num_rows($res_verificar) > 0) {
        ...
        // Guardamos los datos en la sesión para el proceso
        $_SESSION['correo_en_proceso'] = $correo_recuperar;
        $_SESSION['token_generado'] = $token;
        $_SESSION['recuperando_fase'] = 2; // Avanza a la fase de validar código
        
        // Simulamos el envío del correo mostrando el token en pantalla
        echo "<script>
            alert('📩 [SIMULACIÓN DE CORREO ENVIADO VALIDADOR]\\n\\nDe: seguridad@mentalrefuge.com\\nPara: $correo_recuperar\\n\\nTu código de seguridad de 6 dígitos para restablecer tu cuenta es: $token');
        </script>";
    } else {
        echo "<script>alert('El correo ingresado no está registrado en el Refugio.');</script>";
    }
}

// PASO 2: El usuario ingresa el código recibido
if (isset($_POST['btn_validar_codigo'])) {
    $codigo_ingresado = $_POST['codigo_token'];
    
    if ($codigo_ingresado == $_SESSION['token_generado']) {
        $_SESSION['recuperando_fase'] = 3; // Código correcto, avanza a cambiar clave
    } else {
        echo "<script>alert('❌ El código ingresado es incorrecto. Inténtalo de nuevo.');</script>";
    }
}

// PASO 3: El usuario ingresa su nueva contraseña
if (isset($_POST['btn_actualizar_password_real'])) {
    $nueva_password = mysqli_real_escape_string($conexion, $_POST['nueva_password']);
    $correo_destino = $_SESSION['correo_en_proceso'];
    
    $sql_update_pass = "UPDATE v3_usuarios SET password = '$nueva_password' WHERE correo = '$correo_destino'";
    
    if (mysqli_query($conexion, $sql_update_pass)) {
        // Limpiamos las variables de recuperación
        unset($_SESSION['correo_en_proceso']);
        unset($_SESSION['token_generado']);
        unset($_SESSION['recuperando_fase']);
        
        echo "<script>alert('¡Contraseña restablecida con éxito! ✨ Ya puedes ingresar al Refugio.'); window.location='index.php';</script>";
        exit();
    } else {
...
$mostrar_recuperar = isset($_SESSION['recuperando_fase']);
if ($mostrar_recuperar) {
    $fase_recuperar = $_SESSION['recuperando_fase'];
}
...
if (isset($_GET['nuevo_avatar'])) {
    ...
    if (mysqli_query($conexion, $sql_avatar)) {
        $_SESSION['foto_actual'] = $avatar_url;
...
<div id="login-container" <?php if($login_exitoso) echo 'style="display:none;"'; ?>>
    <div class="login-card shadow-lg mx-auto">
        ...
        <?php if ($mostrar_recuperar) { ?>
            <!-- FORMULARIO DE RECUPERACIÓN EN 3 FASES -->
            <?php if ($fase_recuperar == 1) { ?>
                <div class="text-center mb-4">
                    <h5 class="fw-bold text-primary">Recuperar Cuenta</h5>
                    <p class="text-muted small">Ingresa tu correo para enviarte un código de seguridad.</p>
                </div>
                <form method="POST">
                    <div class="text-start mb-3">
                        <label class="small fw-bold mb-1 ms-2">Correo Electrónico</label>
                        <input type="email" name="correo_recuperar" class="form-control btn-round bg-light border-0 p-3" placeholder="tu@correo.com" required>
                    </div>
                    <button type="submit" name="btn_enviar_codigo" class="btn btn-main w-100 btn-round shadow-lg py-3 mb-3">Enviar Código de Seguridad</button>
                    <div class="text-center">
                        <a href="index.php" class="small text-decoration-none text-muted">Cancelar y regresar</a>
                    </div>
                </form>
            <?php } elseif ($fase_recuperar == 2) { ?>
                <div class="text-center mb-4">
                    <h5 class="fw-bold text-success">🔑 Verificar Código</h5>
                    <p class="text-muted small">Hemos "enviado" un código de 6 dígitos a su correo electrónico: <br><strong><?php echo $_SESSION['correo_en_proceso'] ?? ''; ?></strong></p>
                </div>
                <form method="POST">
                    <div class="text-start mb-3">
                        <label class="small fw-bold mb-1 ms-2">Introduce el Código de 6 dígitos</label>
                        <input type="number" name="codigo_token" class="form-control btn-round bg-light border-0 p-3 text-center fw-bold fs-4" placeholder="000000" min="100000" max="999999" required>
                    </div>
                    <button type="submit" name="btn_validar_codigo" class="btn btn-success w-100 btn-round shadow-lg py-3 mb-3">Verificar Código</button>
                </form>
            <?php } elseif ($fase_recuperar == 3) { ?>
                <div class="text-center mb-4">
                    <h5 class="fw-bold text-warning">🛠️ Nueva Contraseña</h5>
                    <p class="text-muted small">Código verificado con éxito. Define tu nueva contraseña de acceso.</p>
                </div>
                <form method="POST">
                    <div class="text-start mb-4">
                        <label class="small fw-bold mb-1 ms-2">Escribe tu nueva clave</label>
                        <input type="password" name="nueva_password" class="form-control btn-round bg-light border-0 p-3" placeholder="Mínimo 8 caracteres••••••••" required>
                    </div>
                    <button type="submit" name="btn_actualizar_password_real" class="btn btn-main w-100 btn-round shadow-lg py-3 mb-3">Restablecer Contraseña</button>
                </form>
            <?php } ?>
        <?php } else { ?>
    ...
    <div class="input-group">
        <input type="password" name="password" id="password" class="form-control btn-round bg-light border-0 p-3" placeholder="••••••••" style="border-top-right-radius: 0px !important; border-bottom-right-radius: 0px !important;" required>
        <span class="input-group-text bg-light border-0 px-3" id="togglePassword" style="cursor: pointer; border-top-right-radius: 30px; border-bottom-right-radius: 30px;">
            ...
            <div class="d-flex justify-content-between px-2 mb-4">
                <a href="index.php?accion=recuperar" class="small text-decoration-none text-primary">Olvidé mi clave</a>
    ...
    <div class="row g-0">
        <nav class="col-md-3 col-lg-2 sidebar p-4 shadow-sm min-vh-100">
            <div class="text-center mb-5">
                <img src="<?php echo isset($_SESSION['foto_actual']) ? $_SESSION['foto_actual'] : 'https://cdn-icons-png.flaticon.com/512/6997/6997662.png'; ?>" id="nav-avatar" class="avatar-img-nav mb-2 shadow" style="width: 50px; height: 50px; border-radius: 50%; object-fit: cover;">
            ...
            </div> <!-- 🛠️ AQUÍ QUEDÓ REPARADA LA ETIQUETA QUE ESTABA ABIERTA -->
        </nav>
                                            ...
                                            <div class="card p-3 mb-2 shadow-sm border-0 bg-light" style="border-radius: 15px;">
                                                <div class="d-flex justify-content-between align-items-center">
                                                    ...
                                                    <div>
                                                        ...
                                                        <a href="index.php?eliminar_diario=<?php echo $fila['id_diario']; ?>" class="btn btn-sm text-danger border-0" onclick="return confirm('¿Seguro que quieres borrar este pensamiento?')">
                ...
                <div class="p-4">
                    ...
                    <div class="row">
                        <div class="col-md-12 text-center">
                            <img src="<?php echo isset($_SESSION['foto_actual']) ? $_SESSION['foto_actual'] : 'https://cdn-icons-png.flaticon.com/512/6997/6997662.png'; ?>" id="big-avatar" class="avatar-img-big mb-3 shadow" style="width: 150px; height: 150px; border-radius: 50%; object-fit: cover;">                            
                            ...
                            <p class="small text-muted mt-3">O elige un avatar inclusivo:</p>
                            ...
                            <div class="card card-custom p-4 shadow-sm border-0 mb-4 text-start">
                                ...
                                <form method="POST">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="nuevo_apodo" class="form-control border-primary" value="<?php echo $_SESSION['apodo_usuario'] ?? ''; ?>" placeholder="Ej: Kathe">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <input type="text" class="form-control bg-light border-0 text-muted" value="<?php echo $_SESSION['nombre_usuario']; ?>" readonly>
                                    </div>
                                    <div class="mb-3">
                                        <label class="small text-muted fw-bold">Correo Electrónico</label>
                                        <input type="email" class="form-control bg-light border-0 text-muted" value="<?php echo $_SESSION['correo_usuario']; ?>" readonly>
                                    </div>
                                    <button type="submit" name="btn_actualizar_apodo" class="btn btn-main btn-sm w-100 btn-round">Guardar mi Apodo</button>
                                </form>
                            </div>

                            <div class="card card-custom p-4 shadow-sm border-0 text-start">
...
// Lógica para mostrar/ocultar la contraseña con el ojito
